...
| 저장 슬롯 | 슬롯 3개 선택·생성·삭제, 생성 때 저장 이름 지정 |
| 로비 | 재화 표시, 메뉴 진입, 리더 standing, 설정, 배경·standing 꾸미기, 로비 BGM 플레이리스트 |
| 스테이지 | 스테이지 선택과 최근 전투 결과, 깬 스테이지의 전투 BGM 선택 |
| 덱 구성 / 장비 / 성장 | 공통 작업대 UI를 공유한다 |
| 전장 | 배치·이동·공격·스킬·턴 진행·방어 선택·연출·연출 배속·결과 저장 |

전장·덱 구성·장비·성장은 **카드 정보 패널**을 공유한다. 카드를 길게 누르거나 우클릭하면
능력·특성·설명이 열리고, 그림을 누르면 원화를 크게 본다.

**소리**는 로그인부터 로비까지 Main BGM이 이어지고, 로비와 전투는 각자 고른 곡으로 바뀐다.
볼륨은 전체·음악·효과음·음성 네 채널이며 저장 슬롯이 아니라 기기에 저장한다.
효과음은 자산이 아직 없어 재생기만 준비돼 있다.

### 아직 없는 것

가장 큰 공백은 **방어 선택**이다. 엔진과 UI는 준비돼 있는데 `guardian_block` 능력을 가진 카드가
테스트 덱에만 있어 실전에서 발동하지 않는다.
