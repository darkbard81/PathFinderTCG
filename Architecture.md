...
**`PUT /api/save-slots/:id`가 아직 진행도를 받는다.** 전투 보상은 서버가 적지만, 그 뒤에 브라우저가
`exp`·`resources`·`clearedStageIds`를 덮어써 보내면 그대로 저장된다. EXP는 레벨을 통해 전투 수치로
이어진다. 성장표 상한이 있어 최악이 '전부 만렙'이지 임의의 수치는 아니다.
