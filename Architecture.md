...
**EXP는 아직 클라이언트가 정한다.** 수치는 정의와 레벨에서 다시 계산하지만 EXP 자체는 저장본 값을 쓴다.
EXP를 올려 보내면 레벨이 오르고 그만큼 수치도 오른다. 다만 성장표 상한을 넘지는 못해,
최악이 '전부 만렙'이지 임의의 수치는 아니다. 성장과 보상을 서버가 소유해야 닫힌다.
