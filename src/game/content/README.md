...
- `STAGE_ONE_REWARD_ENTRIES`: Phase 8에서 Stage 01에 연결할 적 카드 레어리티별 보상 가중치
- `validateTestCardPool`: 비용·역할·Skill·레어리티·예산·상태 규칙·아트 입력 분포 검증

서버는 `createPhaseThreeGameContent`를 기본 콘텐츠로 사용한다. 새 슬롯마다 새 소유 인스턴스와
덱 ID를 발급하며, Phase 1 fixture를 런타임 데이터로 사용하지 않는다.
...
`STAGE_ONE_REWARD_ENTRIES`는 승인된 가중치 데이터만 제공한다. Stage 실행, 보상 추첨과 멱등
지급은 Phase 8 범위다.
