...
브라우저에서 `http://127.0.0.1:3010`을 열면 Login 화면이 나타난다. `npm run dev`만 실행해도
화면은 열리지만 인증·세이브 동작에는 `http://127.0.0.1:3011` API가 필요하다. UI showcase
Scene은 개발 참고용으로 등록만 유지하고 기본 진입점으로 사용하지 않는다.
