...
| 구현 범위 | 전투 준비·턴·6 Action·Skill/Effect·상태 확인·승패·결정적 Stage 01 AI |
...
`BattleSession`은 Action 하나와 그에 연결된 Active/Reactive Skill, 상태 확인, 턴 종료와 다음
턴 시작까지 동기식으로 해결한다.
...
| 합법 행동 규칙 | [`rules.ts`](../../src/game/simulation/battle/rules.ts)                                                                                                                                                                                                                                                                        | 6 Action 생성·검사, PLACE·ATTACK·MOVE 규칙              |
...
SavedDeck / EnemyDeckBlueprint
  ...
  -> TURN_STARTED, 첫 게임 턴 의무 드로우 생략
  -> BattleSession ACTION phase
```

`BattleState`는 다음 정보만 가진다.

- 양쪽 Deck, Hand, 6칸 Field, Drop, Exile의 전투 카드 ID
- 카드 정의 ID, 소유자, 원본 출처, 피해, `EXILED`, 등장 대기, 비영구 수치 변경
...
| 규칙 영역               | 구현 결과                                                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 턴                      | `TURN_STARTED` → 의무 DRAW → Action 하나 → `TURN_ENDED` → 손패 제한 → 상대 턴을 고정 순서로 해결                         |
| 첫 턴                   | Stage 01 기본값으로 `PLAYER`가 먼저 행동하고 첫 게임 턴의 의무 드로우만 생략                                             |
...
| DRAW                    | 일반 DRAW와 선택 Field Active 출처를 구분하고 빈 Deck 선언을 거부                                                        |
| PLACE                   | Hand 유닛, 빈 Field, 선언 시 유효 cost 이하 지배력, 선택 지원 Active를 검사                                              |
| MOVE                    | 등장 대기가 아닌 Field 카드가 자기 진영의 인접한 빈 칸으로만 이동                                                        |
| ATTACK                  | 전열 정면·대각선, 정면 전열이 비었을 때만 바로 뒤 후열, 대각선 후열 금지, 등장 대기 방어자 반격 금지                     |
| DISCARD                 | Hand 유닛을 Exile로 보낸 뒤 한 장 DRAW하며 선택 지원 Active를 해결                                                       |
| END_TURN                | 항상 합법이고 선택 Active가 있으면 먼저 해결                                                                             |
...
7. 유효 피해는 Action 전후 Field에 남은 누적 피해의 순증가다. 파괴된 유닛은 피해 점수 대신
   파괴 점수만 받는다.
8. 한 Action의 최종 상태는 턴 종료, 손패 조정, 상대 `TURN_STARTED`와 의무 DRAW까지 끝나 다음
   Action을 바로 받을 수 있는 상태로 정했다.
...
- 게이트 E: 새 Action, Trigger, Effect, 상태, 존, 카드 종류와 추가 Stage를 만들지 않는 v1 동결
...
끝나는지 확인한다. 별도 진단으로 seed 1–12를 실행했을 때 모두 19–44 Action에 종료됐고
PLAYER 5승, ENEMY 7승이었으며 종료 원인은 모두 Deck 소진이었다.
...
넘긴 seed와 리더 전투 패배가 없었던 점은 Phase 7 화면 통합 후 실제 대전 로그에서 다시
확인해야 한다. 카드 수치나 AI 점수를 즉시 임의 조정하지 않는다.
...
항복과 토너먼트 시간 종료 판정은 여섯 카드 Action이 아닌 외부 매치 제어이므로 이번
...
| 검증                                      | 결과                                           |
| ----------------------------------------- | ---------------------------------------------- |
| Phase 5 targeted 테스트                   | `PASS` — 4 files, 29 tests                     |
| 시작 Hand·교환·첫 턴 재현                 | `PASS`                                         |
| 6 Action 합법·불법 사례                   | `PASS`                                         |
...
| `npm test`                                | `PASS` — 23 files, 108 tests                   |
| `npm run build`                           | `PASS`                                         |
| `git diff --check`                        | `PASS`                                         |

순수 simulation과 문서만 변경했으며 Phaser/rexUI 화면·자산·스타일에는 변경이 없으므로 가로·세로
브라우저 시각 검증 대상은 아니다.
